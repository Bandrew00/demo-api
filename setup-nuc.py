...
if __name__ == "__main__":
   ...
   try:
	  ...
	  ssh_command  = 'ssh ' + c_nuc_ssh_user + nuc_ip                      # comando SSH con credenziali utente NUC (ssh tiesse@xx.xx.xx.xx)
          ...
          # Path NUC
          #
          nuc_hostname = sp.check_output(ssh_command + ' hostname',shell=True) # ricavo nome host della nuc
 	  nuc_folder   = nuc_hostname.replace('\n','')+"_"+nuc_ip+'/'          # cartella NUC da creare
	      ...
	      ip         = ip[0:ip.rfind('/')]
              ...
              #
              nic_hostname = sp.check_output(ssh_command + ' ssh root@' + ip + ' hostname',shell=True) # ricavo nome host della nic              
